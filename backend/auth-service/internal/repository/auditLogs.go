...
func (repository *AuditLogRepositoryImplementation) GetAuditLogsById(id int) (*models.AuditLogs, error) {
	...
	query := `SELECT id, requestId, requestIp, requestTimestamp, userId, action, resource, extra FROM auditlogs WHERE id = $1`
...
func (repository *AuditLogRepositoryImplementation) CreateAuditLog(auditLog *models.AuditLogs) (*models.AuditLogs, error) {
	query := `INSERT INTO auditlogs (requestId, requestIp, requestTimestamp, userId, action, resource, extra) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`
